...
import { Award, MapPin, Languages, Shield, ArrowLeft, Send } from 'lucide-react';
import { jojApi, type OfficialProvider } from '../lib/jojApi';

export default function JojProviderDetailPage() {
  const { id } = useParams();
  ...
  const loadProvider = async () => {
    try {
      setLoading(true);
      const data = await jojApi.getOfficialProvider(Number(id));
      setProvider(data);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-purple-50 py-12 px-4">
      <div className="max-w-5xl mx-auto">
        <Link to="/joj/official-providers">
