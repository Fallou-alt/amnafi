...
import { Star, MapPin, Phone, ArrowLeft, MessageCircle, CheckCircle, Award, Shield, User } from 'lucide-react';
...
export default function OfficialProviderDetailPage() {
  const { id } = useParams();
  ...
  const fetchProviderDetails = async () => {
    try {
      const response = await api.get(`/public/official-providers/${id}`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <Link to="/prestataires-officiels" className="text-blue-600 hover:underline">Retour</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-orange-50">
      <div className="relative h-64 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500">
        ...
        <div className="absolute top-6 left-6">
          <Link to="/prestataires-officiels" className="flex items-center text-white hover:text-gray-200 bg-black/30 backdrop-blur-sm px-4 py-2 rounded-lg">
